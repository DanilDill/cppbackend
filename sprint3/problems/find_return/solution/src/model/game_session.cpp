...
namespace model
{
    ...
    {
        if (_lostObjects.size() == _players.size())
        {
            return;
        }
        unsigned new_count = _lootGenerator->Generate(ms, _lootCount, _players.size());
    ...
    void GameSession::GenerateLostObjects()
    {
        ...
        while (_lootCount>_lostObjects.size())
        {
            ...
            geom::Point2D coord = { static_cast<double>(_map->GetRoads()[road_num].GetStart().x), static_cast<double>(_map->GetRoads()[road_num].GetStart().y)};
            _lostObjects.push_back(GameItem(type,coord));
        }
    }
    ...
    {
        boost::asio::post(_strand,[player = _players[token], speed = _map->getDogSpeed(),direction]()
        {
    ...
    {
        _players.try_emplace(token, player);
    }
    ...
    {
        auto player = _players.find(token);
